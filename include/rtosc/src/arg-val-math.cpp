                                         ...
                                         int ith, rtosc_arg_val_t* result)
{
    ...
    if( rtosc_arg_val_from_int(&n_as_arg_val, range_arg[1].type, ith) )
    {
        rtosc_arg_val_t mult;
        if( rtosc_arg_val_mult(&n_as_arg_val, range_arg+1, &mult) )
        {
            rtosc_arg_val_add(range_arg+2, &mult, result);
            return result;
        }
        else return nullptr;
        // ^ fix a compiler warning complaining that mult may be uninitialized
    }
    else // the range_arg type can not be multiplied (should never happen)
        return nullptr;
}
